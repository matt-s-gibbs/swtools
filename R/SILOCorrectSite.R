...
data=data.frame(x=zoo::window(rain_reference,start=as.Date(paste0(year_start,"-01-01")),end=as.Date(paste0(year_break,"-01-01"))),
                y=zoo::window(rain_correct,start=as.Date(paste0(year_start,"-01-01")),end=as.Date(paste0(year_break,"-01-01"))))
lm_first<-lm(y~x,data=data)

data<-data.frame(x=zoo::window(rain_reference,start=as.Date(paste0(year_break,"-01-01")),end=as.Date(paste0(year_end,"-01-01"))),
                 y=zoo::window(rain_correct,start=as.Date(paste0(year_break,"-01-01")),end=as.Date(paste0(year_end,"-01-01"))))
lm_second<-lm(y~x,data=data)

plot(rain_reference,rain_correct)
abline(lm_first,col="red")
abline(lm_second,col="blue")
...
x=predict(lm_first,data)-predict(lm_second,data)

corrections<-corrections %>%  dplyr::mutate(correction=x) %>% 
  dplyr::rowwise() %>%
  dplyr::mutate(corrected=ifelse(after,ifelse(year<year_break,corrected-correction,corrected),
                                                           ifelse(year>year_break,corrected+correction,corrected))) %>% 
  dplyr::ungroup() %>% 
  dplyr::mutate(factor=corrected/orig)

#apply to the daily data
daily<-X[[s_correct]]$tsd$Rain
daily<-tibble::tibble(Date=index(daily),Rain=as.numeric(daily))
daily<-daily %>% dplyr::mutate(year=year(Date)) %>% 
  dplyr::left_join(corrections %>%dplyr:: select(year,factor),by="year") %>% 
  dplyr::mutate(Rain=Rain*factor)
